            ...
            {links.map((link) => (
                <div key={link.name}>
                    <div className="px-3 text-left md:cursor-pointer group normal-case">
                        <h1
                            className={` ${navbar ? "md:text-gray-900" : "md:text-white"
                                } text-gray-900  py-7 flex justify-between items-center md:pr-0 pr-5 group hover:text-orange-500 font-semibold`}
                            onClick={() => {
                                        ...
                                        {link.sublinks.map((mysublinks) => (
                                            <div key={mysublinks.Head}>
                                                {mysublinks.sublink.map((slink) => (
                                                    <Link href={slink.link} className="" key={slink.name}>
                                    ...
                                    <div>
                                        {slinks.sublink.map((slink) => (
                                            <li className="py-3 pl-4" key={slink.name}>
                                                <Link href={slink.link} className="text-gray-900">
                                                    {slink.name}
                                                </Link>
                                            </li>
                                        ))}
