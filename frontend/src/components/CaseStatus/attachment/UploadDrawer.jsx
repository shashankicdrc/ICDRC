...
export default function UploadDrawer({ type, id }) {
    ...
    useEffect(() => {
        if (type === 'admin' && id.length) {
            const case_type = searchParms.get('caseType')
            setcaseType(case_type)
            setcaseId(id)
        }
    }, [searchParms, type, caseId])
    ...
    const uploadHandler = async (value) => {
        try {
            ...
            formData.set("type", type === 'admin' ? caseType : caseData[1]);
            formData.set("id", type === 'admin' ? caseId : caseData[0]._id);
            formData.set("attachment_type", type === 'admin' ? 'icdrc' : "user")
