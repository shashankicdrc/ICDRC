...
const SocialIcons = () => {
        ...
        },
        {
            ...
            component: (
                <div className='flex flex-col justify-center items-center font-[Roboto] text-sm'>
                    <Link to="/register" className="max-w-max rounded-md px-3.5 py-1 flex justify-center items-center m-1 overflow-hidden relative group cursor-pointer border-2 font-medium border-orange-500 text-indigo-600 hover:text-white">
                    ...
                    <Link to="/partner" className="max-w-max rounded-md px-3.5 py-1 flex justify-center items-center m-1 overflow-hidden relative group cursor-pointer border-2 font-medium border-orange-500 text-indigo-600 hover:text-white">
                        <span className="absolute w-64 h-0 transition-all duration-500 origin-center rotate-45 -translate-x-20 bg-orange-500 top-1/2 group-hover:h-64 group-hover:-translate-y-32 ease"></span>
    ...
    return (
        <div className='main_icon'>
            <div onClick={() => setShowBot((prev) => !prev)} className="main_icon rounded-full h-8 md:h-12 w-8 md:w-12 group bg-white border-2 border-orange-600 hover:bg-orange-600 cursor-pointer fixed bottom-28 md:bottom-36 right-4  flex justify-center items-center transition-all 300 ease-in-out text-orange-600 hover:text-white">
                <TbMessageChatbot className=' text-xl font-semibold md:text-2xl' />
            </div>
            {
                showbot &&
                <div className='fixed bottom-28 right-4 md:right-16 z-20 pr-4 pl-8 md:px-0'>
                    <ChatBot steps={steps}
                        floating={false}
                        headerComponent=
                        {
                            <div className='bg-orange-500 rounded-t-xl p-3 flex justify-between items-center'>
                                <div className='flex items-center'>
                                    <AiFillWechat size={30} className="text-white" />
                                    <div className='flex flex-col ml-2 font-[Poppins]'>
                                        <span className='text-sm font-semibold text-white'>RaksaBot (रक्षा बोट)</span>
                                        <span className='text-xs font-base text-white'>Customer Communication</span>
                                    </div>
                                </div>
                                <div className='cursor-pointer' onClick={() => setShowBot(false)}>
                                    <ImCross className='text-white' size={20} />
                                </div>
                            </div>
                        }
                        botAvatar="https://res.cloudinary.com/dl5hosmxb/image/upload/v1691047910/chatbot/Untitled_design_bidika.svg"
                        bubbleStyle={{
                            background: '#f97316', fontFamily: 'roboto', color: 'white', maxWidth: '60%'
                        }}
                    />
                </div>
            }
