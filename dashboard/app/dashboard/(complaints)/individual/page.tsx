...
export default async function page({ searchParams }: Props) {
    ...
    const url = `${BASE_URL}/api/admin/individual/complaints?page=${page}&perRow=${perRow}${filters}${sorts}`;
    const { data, error } = await getIndividualCase(token, url);
    if (error) {
        return (
            ...
            <Card>
                <CardContent>
                    <ErrorBoundary FallbackComponent={ErrorFallback}>
                        <IndividualDataTable columns={CaseColumns} data={data.complaints} />
                    </ErrorBoundary>
