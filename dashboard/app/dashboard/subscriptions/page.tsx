...
export default async function page({ searchParams }: Props) {
    ...
    const url = `${BASE_URL}/api/admin/subscription?page=${page}&perRow=${perRow}${filters}${sorts}`;
    const { data, error } = await getSubscriptions(token, url);
    if (error) {
        return (
            ...
            <Card>
                <CardContent>
                    <ErrorBoundary FallbackComponent={ErrorFallback}>
                        <SubscriptionDataTable columns={SubscriptionColumns} data={data.subscriptions} />
                    </ErrorBoundary>
