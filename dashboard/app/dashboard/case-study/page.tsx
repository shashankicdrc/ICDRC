...
import { getCaseStudys } from "@/externalAPI/caseStudyService";
...
export default async function page({ searchParams }: Props) {
    ...
    const url = `${BASE_URL}/api/case-study?page=${page}&perRow=${perRow}${filters}${sorts}`;

    const { data, error } = await getCaseStudys(token, url);
    ...
    return (
        <main className="p-4 sm:px-6 sm:py-0 md:gap-8">
            <div className="flex items-center justify-between">
                <h1 className="my-2 text-4xl font-semibold">Manage Case Studies</h1>
            </div>
            <div className="flex  items-center justify-between">
                ...
                <Button asChild>
                    <Link href="/dashboard/case-study/add">Add Case Study</Link>
                </Button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 my-5">
                {data.caseStudy.length < 1 ? <div className="flex justify-center text-center mx-auto my-5 col-span-3">
                    <p className="font-bold">No Case Study found</p>
                </div>
                    : data.caseStudy.map((caseStudy: blogType) => <CaseStudyItem data={caseStudy} key={caseStudy._id} />)}
            </div>
