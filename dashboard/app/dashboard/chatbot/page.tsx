import { ChatBotColumns } from '@/components/chatbot/columns';
import { ChatBotDataTable } from '@/components/chatbot/DataTable';
...
import { getChatBots } from '@/externalAPI/chatBotService';
...
export default async function page({ searchParams }: Props) {
    ...
    const url = `${BASE_URL}/api/chat-bots?page=${page}&perRow=${perRow}${filters}${sorts}`;
    const { data, error } = await getChatBots(token, url);
    if (error) {
        return (
            ...
            <Card>
                <CardContent>
                    <ErrorBoundary FallbackComponent={ErrorFallback}>
                        <ChatBotDataTable columns={ChatBotColumns} data={data.chatbots} />
                    </ErrorBoundary>
