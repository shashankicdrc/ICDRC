...
import { ContactDataTable } from '@/components/contacts/DataTable';
...
import { getContacts } from '@/externalAPI/contactService';
...
export default async function page({ searchParams }: Props) {
    ...
    const url = `${BASE_URL}/api/contacts?page=${page}&perRow=${perRow}${filters}${sorts}`;
    const { data, error } = await getContacts(url);
    if (error) {
        return (
            ...
            <Card>
                <CardContent>
                    <ErrorBoundary FallbackComponent={ErrorFallback}>
                        <ContactDataTable columns={ContactColumns} data={data.contacts} />
                    </ErrorBoundary>
