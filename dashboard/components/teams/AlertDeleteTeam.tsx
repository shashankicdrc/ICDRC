...
import { deleteCaseStudyAction, deleteTeamAction } from "@/action";

interface Props {
    teamIds: string[];
}

export const AlertDeleteTeams = ({ teamIds }: Props) => {
    ...
    const onClick = async () => {
        try {
            ...
            const { error, message } = await deleteTeamAction(token, teamIds);
            setisLoading((prevState) => !prevState);
            if (error) {
